...
// every block not visited when starting from a traversal for
// blocks sitting on the floor and skipping the block to be
// disintegrated must be supported only by the disintegrated
// brick and the bricks it supports, and will collapse
...
public:
    ...
    jengaBlock(int firstRangeX, int firstRangeY, int firstRangeZ, int secondRangeX, int secondRangeY, int secondRangeZ, int id, axis side) {
...
int main(int argc, char* argv[]) {
    ...
    std::sort(blocks.begin(), blocks.end(), comparator);
    findSupporters();  
    std::cout << "Number of bricks that will fall: " << numBricksDisintegrated() << "\n";
}
...
void processLine(std::string s) {
    ...
    if (firstX != secondX)
        ...
        side = Z;
    blocks.push_back(new jengaBlock(firstX, firstY, firstZ, secondX, secondY, secondZ, globalId, side));
    globalId++;
}
...
long long numBricksDisintegrated() {
    long long totalBricks = blocks.size();
    long long droppedBricks = 0;
    for (int i = 0; i < blocks.size(); i++) {
        visitedBlocks.clear();
        std::deque<jengaBlock*> toVisit = {};
        for (int j = 0; j < blocks.size(); j++) {
            if (blocks[j]->firstRangeZ == 1) {
                visitedBlocks[blocks[j]->id] = 1;
            }
            if (j != i and blocks[j]->firstRangeZ == 1) {
                toVisit.push_back(blocks[j]);
            }
        }
        while (!toVisit.empty()) {
            jengaBlock* b = toVisit.front();
            toVisit.pop_front();
            for (jengaBlock* c : b->supportedBlocks) {
                if (c->id != blocks[i]->id and !visitedBlocks.count(c->id))
                    toVisit.push_back(c);
                    visitedBlocks[c->id] = 1;
            }
        }
        long long numVisited = 0;
        for (std::map<int, int>::iterator it = visitedBlocks.begin(); it != visitedBlocks.end(); ++it) {
            numVisited++;
        }
        droppedBricks += totalBricks - numVisited;
    }
